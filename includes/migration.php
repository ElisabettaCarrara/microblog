...
if ( ! defined( 'MY_MICROBLOG_PLUGIN_VERSION' ) ) {
	define( 'MY_MICROBLOG_PLUGIN_VERSION', '2.0.0' ); // This should match your plugin header Version.
}
if ( ! defined( 'MY_MICROBLOG_V2_MIGRATION_VERSION' ) ) {
	define( 'MY_MICROBLOG_V2_MIGRATION_VERSION', '2.0.0' ); // The first v2 version requiring these CPT/taxonomy changes.
}
if ( ! defined( 'MY_MICROBLOG_DB_VERSION_OPTION_KEY' ) ) {
	define( 'MY_MICROBLOG_DB_VERSION_OPTION_KEY', 'my_microblog_db_version' ); // Option key for plugin's data/database version.
}
if ( ! defined( 'MY_MICROBLOG_ORPHAN_CHECK_DONE_FLAG' ) ) {
	define( 'MY_MICROBLOG_ORPHAN_CHECK_DONE_FLAG', 'my_microblog_v2_orphaned_check_done' ); // Flag for the orphan check.
}
if ( ! defined( 'MY_MICROBLOG_MIGRATION_V1_V2_FLAG' ) ) {
...
if ( ! defined( 'MY_MICROBLOG_TEXT_DOMAIN' ) ) {
	define( 'MY_MICROBLOG_TEXT_DOMAIN', 'my-microblog-plugin' ); // Replace with your plugin's text domain.
}
 ...
 * @since 2.0.0
 */
function my_microblog_on_activate() {
	if ( false === get_option( MY_MICROBLOG_DB_VERSION_OPTION_KEY ) ) {
		update_option( MY_MICROBLOG_DB_VERSION_OPTION_KEY, MY_MICROBLOG_PLUGIN_VERSION );
	}
}
// Assuming your main plugin file is 'my-microblog-plugin.php', replace __FILE__ if this code is elsewhere.
// register_activation_hook( __FILE__, 'my_microblog_on_activate' );
 ...
 * @since 2.0.0
 */
function my_microblog_check_for_orphaned_v1_data_and_migrate() {
	// Only run this specific check once.
	if ( get_option( MY_MICROBLOG_ORPHAN_CHECK_DONE_FLAG ) ) {
		return;
	}

	// If the main v1->v2 migration has already run (e.g., via a standard upgrade path),
	// mark orphan check as done and exit.
	if ( get_option( MY_MICROBLOG_MIGRATION_V1_V2_FLAG ) ) {
		update_option( MY_MICROBLOG_ORPHAN_CHECK_DONE_FLAG, true );
		return;
	}
...
add_action( 'admin_init', 'my_microblog_check_for_orphaned_v1_data_and_migrate', 5 );
...
function my_microblog_check_for_upgrade() {
	...
	// If MY_MICROBLOG_DB_VERSION_OPTION_KEY is not set, it's likely a fresh install,
	...
	if ( false === $current_db_version ) {
		...
		// If the orphan check *did* migrate, the MY_MICROBLOG_MIGRATION_V1_V2_FLAG would be set.
		...
		update_option( MY_MICROBLOG_DB_VERSION_OPTION_KEY, MY_MICROBLOG_PLUGIN_VERSION );
		$current_db_version = MY_MICROBLOG_PLUGIN_VERSION; // Set for further logic in this same execution.
	}
...
add_action( 'admin_init', 'my_microblog_check_for_upgrade', 10 );
...
function run_microblog_v1_to_v2_migration() {
	...
	$v2_post_type = 'cp_microblog'; // As per previous discussions.
	$v1_taxonomy  = 'microblog_tag';
	$v2_taxonomy  = 'microblog_category'; // As per previous discussions.
	...
	if ( empty( $old_post_ids ) ) {
		...
		update_option( MY_MICROBLOG_MIGRATION_V1_V2_FLAG, true );
		return;
	}
		...
		function() {
			...
			<div class="notice notice-success is-dismissible">
				<p>
					<?php
					printf(
						/* translators: %s: plugin name (not available here, so hardcode or retrieve if possible) */
						esc_html__( 'Microblog plugin data has been successfully migrated to the new format. If you encounter any issues, please check our support channels.', MY_MICROBLOG_TEXT_DOMAIN )
					);
					?>
				</p>
			</div>
...
/**
 * Placeholder for plugin activation hook registration.
 * This should be in your main plugin file.
 *
 * Example:
 * register_activation_hook( __FILE__, 'my_microblog_on_activate' );
 */
